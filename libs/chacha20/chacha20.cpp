...
int encrypt(const uint8_t* in, size_t inLen, const uint8_t* key, const uint8_t* nonce, uint8_t* out) {
    try {
        if (!in || !key || !nonce || !out) {
            std::cerr << "ChaCha20: ошибка — нулевой указатель" << std::endl;
            return -1;
        }
        if (inLen == 0) {
            std::cerr << "ChaCha20: ошибка — пустой вход" << std::endl;
            return -1;
        }
...
void generateKey(uint8_t* key) {
    try {
        if (!key) {
            std::cerr << "ChaCha20: ошибка — нулевой указатель ключа" << std::endl;
            return;
        }
        srand(time(0));
        for (int i = 0; i < 32; i++)
