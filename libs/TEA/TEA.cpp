...
namespace
{
    ...
    std::array<uint32_t, 4> parse_key(const char* key)
    {
        std::string s(key);
    ...
    void encrypt_block(uint32_t& v0, uint32_t& v1, const std::array<uint32_t, 4>& key)
    {
        uint32_t sum = 0;
    ...
    void decrypt_block(uint32_t& v0, uint32_t& v1, const std::array<uint32_t, 4>& key)
    {
    ...
    std::vector<uint8_t> add_padding(const uint8_t* data, uint64_t size)
    {
    ...
    std::vector<uint8_t> remove_padding(const uint8_t* data, uint64_t size)
    {
        if (size == 0)
        {
...
extern "C"
{
    ...
    {
        std::array<uint32_t, 4> tea_key = parse_key(key);
        std::vector<uint8_t> buf = add_padding(data, size);

        for (size_t i = 0; i < buf.size(); i += 8)
        {
            uint32_t v0 = bytes_to_uint32(&buf[i]);
            uint32_t v1 = bytes_to_uint32(&buf[i + 4]);

            encrypt_block(v0, v1, tea_key);

            uint32_to_bytes(v0, &buf[i]);
            uint32_to_bytes(v1, &buf[i + 4]);
        }
    ...
    CipherResult decrypt(const uint8_t* data, uint64_t size, const char* key)
    {
        ...
        for (size_t i = 0; i < buf.size(); i += 8)
        {
            ...
            decrypt_block(v0, v1, tea_key);

            uint32_to_bytes(v0, &buf[i]);
            uint32_to_bytes(v1, &buf[i + 4]);
        }

        std::vector<uint8_t> unpadded = remove_padding(buf.data(), buf.size());
